...
def resolve_archive_path(
    zip_name: str, origin: Optional[str]
    ...
    if len(candidates) == 1:
        ...
        cats = ", ".join(
            [p.split("/", 2)[1] for p, _ in candidates]
        )  # nombres de categoría
        print(
            f"[ERROR] ZIP '{zip_name}' exists in multiple categories: {cats}. "
            f"Please specify --origin.",
            file=sys.stderr,
        )
        sys.exit(2)
...
def write_delete_log(
    ...
    cat = ""
    if dest_prefix.startswith(ARCHIVE_PREFIX) and len(dest_prefix) > len(
        ARCHIVE_PREFIX
    ):
...
def bq_soft_delete(
    zip_name: str, gcs_uri: str, reason: str, who: str, generation: Optional[int]
) -> int:
    """
...
def main() -> None:
    ...
    parser.add_argument(
        "--zip", required=True, help="ZIP file name (e.g., my_dataset_2025-08-31.zip)"
    )
    parser.add_argument(
        "--reason", default="cleanup", help="Reason for deletion (default: cleanup)"
    )
    parser.add_argument(
        "--origin",
        choices=ORIGIN_CHOICES,
        help="Category where the ZIP lives (public|simulated|real|youtube). If omitted, it will be auto-resolved.",
    )
    parser.add_argument(
        "--who", default="uploader-cli", help="Actor performing the deletion"
    )
    ...
    existed_before, generation, gcs_uri, dest_prefix = gcs_delete_zip(
        zip_name, args.origin
    )
    ...
    effective_reason = (
        args.reason if existed_before else f"{args.reason}|object-not-found"
    )
