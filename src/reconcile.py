...
def list_gcs_archive_objects() -> Dict[str, List[Tuple[str, Optional[int], str]]]:
    ...
    for blob in client.list_blobs(BUCKET, prefix=ARCHIVE_PREFIX):
        ...
        rel = name[len(ARCHIVE_PREFIX):]  # p.ej. 'public/LISA_images.zip' o 'LISA_images.zip'
        if "/" in rel:
...
def bq_soft_delete_zip(zip_name: str, reason: str, who: str, dry_run: bool) -> None:
    ...
    """
    if dry_run:
        print(f"[DRY-RUN] Soft-delete BQ: zip_name={zip_name}, reason={reason}, who={who}")
        return
    bq = get_bq()
    table_ref_full = f"{PROJECT}.{DATASET}.{TABLE}"
    bq.query(
...
def bq_mark_exists(zip_name: str, generation: Optional[int], new_gcs_uri: Optional[str], dry_run: bool) -> None:
    """
    Marca exists_in_gcs=TRUE en BQ para un zip_name activo
    si estaba en FALSE o NULL y el objeto existe en GCS.
    """
    if dry_run:
        print(f"[DRY-RUN] Mark exists_in_gcs=TRUE: zip_name={zip_name}, gen={generation}, uri={new_gcs_uri or '<keep>'}")
        return
    bq = get_bq()
    table_ref_full = f"{PROJECT}.{DATASET}.{TABLE}"
    bq.query(
...
def bq_reactivate_deleted(zip_name: str, generation: Optional[int], new_gcs_uri: Optional[str],
                            who: str, dry_run: bool) -> None:
    """
    Reactiva una fila soft-deleted cuando el ZIP existe en GCS.
    """
    if dry_run:
        print(f"[DRY-RUN] Reactivate BQ row: {zip_name}, gen={generation}, uri={new_gcs_uri or '<keep>'}, by={who}")
        return
    bq = get_bq()
    table_ref_full = f"{PROJECT}.{DATASET}.{TABLE}"
    bq.query(
...
def write_reconcile_log(payload: dict, upload: bool, filename: Optional[str] = None) -> None:
    """
...
def main() -> None:
    ...
    ap.add_argument("--reason", default="reconcile-missing",
                    help="Soft-delete reason to set when ZIP is missing in GCS")
    ap.add_argument("--who", default="reconcile-cli",
                    help="Actor label for updates")
    ap.add_argument("--upload-log", action="store_true",
    ...
    ap.add_argument("--reactivate-deleted", action="store_true",
    ...
    duplicates_basenames: Set[str] = {bn for bn, entries in gcs_index.items() if len(entries) > 1}
    ...
    untracked_in_bq: List[str] = sorted([bn for bn in gcs_basenames if bn not in {r["zip_name"] for r in rows_active}])
    ...
    for r in rows_active:
        ...
        resolved_path, generation, status = resolve_gcs_match(zip_name, bq_uri, gcs_index)

        if status == "not_found":
            missing_in_gcs.append(zip_name)
            bq_soft_delete_zip(zip_name, reason=args.reason, who=args.who, dry_run=args.dry_run)
            continue
        ...
        if exists_flag in (False, None):
            bq_mark_exists(zip_name, generation=generation, new_gcs_uri=real_uri, dry_run=args.dry_run)
    ...
    if args.include_deleted:
        ...
        for r in rows_deleted:
            ...
            resolved_path, generation, status = resolve_gcs_match(zip_name, r.get("gcs_uri"), gcs_index)

            if status in ("exact_uri", "unique_basename"):
                real_uri = f"gs://{BUCKET}/{resolved_path}" if resolved_path else None
                if args.reactivate_deleted:
                    bq_reactivate_deleted(zip_name, generation, real_uri, who=args.who, dry_run=args.dry_run)
